...
def hamil(sx, sp, vx, vp, mchi, lam, nu, gg):
    ...
    for i in range(NSITE):
        for j in range(NSITE):
            ...
            h = h + 0.5*sp[i, j]**2 - 0.5*sx[i, j]*div_s + \
                0.5*sum_v + lam*(sx[i, j]**2 - nu**2)**2
    ...
    for k in range(dim):
        for j in range(NSITE):
            for i in range(NSITE):
                ...
                vf[i, j, k] = - vx[i, j, k] + 0.5 * \
                    (div_v - gg*sx[pni, pnj])/mchi**2

                pni = pn[i - mu[k, 0] + 2]
                pnj = pn[j - mu[k, 1] + 2]
                div_v = div_vec(vx, pni, pnj)

                vf[i, j, k] = + vf[i, j, k] - 0.5 * \
                    (div_v - gg*sx[pni, pnj])/mchi**2
...
def div_vec(v, i, j):
    div_v = 0.0
    for k in range(dim):
        div_v += v[pn[i+mu[k, 0]+2], pn[j+mu[k, 1]+2], k] - \
            v[pn[i-mu[k, 0]+2], pn[j-mu[k, 1]+2], k]
    return 0.5*div_v
...
for istep in range(NTHERM + NSKIP*NSAMPLE):
    ...
    sx1, sp1, vx1, vp1 = leapfrog_xpx_md(
        tau, nmd, sx1, sp1, vx1, vp1, mchi, lam, nu, gg)
...
# _CHECK_REVERSE_
    ...
    sx2, sp2, vx2, vp2 = leapfrog_xpx_md(
        tau, nmd, sx2, sp2, vx2, vp2, mchi, lam, nu, gg)
...
#
    ...
    if (istep >= NTHERM and np.mod(istep, NSKIP) == 0):
